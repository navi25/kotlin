...
class LazyScriptClassMemberScope(
    ...
    private val scriptPrimaryConstructor: () -> ClassConstructorDescriptorImpl? = resolveSession.storageManager.createNullableLazyValue {
        ...
        if (baseConstructorDescriptor != null) {
            ...
            val paramsIndexBase = baseConstructorDescriptor.valueParameters.let { if (it.isEmpty()) 0 else it.last().index + 1 }
            val syntheticParameters =
                listOf(implicitReceiversParamType, environmentVarsParamType).mapIndexedNotNull { index: Int, param: Pair<String, KotlinType>? ->
                    if (param == null) null
                    else ValueParameterDescriptorImpl(
                        constructorDescriptor,
                        null,
                        paramsIndexBase + index,
                        Annotations.EMPTY,
